...
$(function () {
  ...
  $.ajax({
    type: "GET",
    url: inter_face+"/api/1.0/notes/"+id,
    dataType: "jsonp",
    jsonp: "callback",
    ...
    success: function (data) {
      if (data.code == 200) {
       ...
       /* $('.noteTitle h2').html(datas.subTitle);*/
        $('.startTime').html(datas.timeOfDeparture);
        $('.costPear').html(datas.perCapitaCost);
        $('.persons').html(datas.numberOfTrips);
        $('.tripDays').html(datas.tripDays);
        $('.history').html(datas.noteDetails);
        $('.articleAuthor p').html('本篇文章来源于'+datas.noteSource+'<br/>作者：'+datas.author);
